...
class GitHubWebhookHandler(BaseHTTPRequestHandler):
    ...
    def run_auto_prioritize(self):
        ...
        try:
            script_path = os.path.join(os.path.dirname(__file__), 'auto-prioritize-failures.sh')
    ...
    def handle_workflow_run_event(self, payload):
        ...
        if action == 'requested':
            ...
            message = f"🔄 **Workflow Running**\n**Name:** {workflow_name}\n**Branch:** {head_branch}\n**URL:** {html_url}"
            color = "warning"
            
        elif action == 'completed':
            if conclusion == 'success':
                message = f"✅ **Workflow Succeeded**\n**Name:** {workflow_name}\n**Branch:** {head_branch}\n**Commit:** {head_sha}\n**Actor:** {actor}\n**URL:** {html_url}"
                color = "good"
                ...
                message = f"⚠️ **Workflow Cancelled**\n**Name:** {workflow_name}\n**Branch:** {head_branch}\n**URL:** {html_url}"
                color = "warning"
            else:
                message = f"❓ **Workflow Completed**\n**Name:** {workflow_name}\n**Status:** {conclusion}\n**Branch:** {head_branch}\n**URL:** {html_url}"
                color = "warning"
        else:
            message = f"📋 **Workflow Event**\n**Action:** {action}\n**Name:** {workflow_name}\n**Status:** {status}\n**URL:** {html_url}"
            color = "good"
...
def main():
    ...
    if SECRET:
        ...
        logger.warning("⚠️  Webhook signature verification disabled (set GITHUB_WEBHOOK_SECRET)")
