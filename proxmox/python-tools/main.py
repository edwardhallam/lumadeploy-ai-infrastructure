...
class ProxmoxManager:
    ...
    def create_container(self, node: str, container_id: int, template: str, 
                        hostname: str, cores: int = 2, memory: int = 2048, 
                        rootfs_size: str = "8G", storage: str = "local-lvm") -> bool:
    ...
    def wait_for_container_status(self, node: str, container_id: int, 
                                target_status: str = "running", timeout: int = 300) -> bool:
    ...
    def download_template(self, node: str, template_url: str, storage: str = "local-lvm") -> bool:
    ...
    def get_template_download_status(self, node: str, storage: str = "local-lvm") -> List[Dict[str, Any]]:
            ...
            for item in response.json()['data']:
                if item.get('content') == 'vztmpl':
                    templates.append({
                        ...
                        'size': item.get('size'),
                        'ctime': item.get('ctime'),
                        'status': 'downloaded' if item.get('size', 0) > 0 else 'downloading'
                    })
