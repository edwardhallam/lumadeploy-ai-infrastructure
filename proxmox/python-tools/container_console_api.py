...
from container_console_service import ContainerConsoleManager, CommandResult
...
def deploy_librechat(container_id):
    ...
    try:
        ...
        deployment_steps = [
            ...
            ("Deploy LibreChat", "docker run -d --name librechat --restart unless-stopped -p 3000:3000 -e PUID=1000 -e PGID=1000 ghcr.io/danny-avila/librechat:latest")
        ]
        ...
        check_result = console_manager.execute_command(container_id, "docker ps | grep librechat", timeout=30)
        librechat_running = "librechat" in check_result.output
        
        # Get container IP
        ip_result = console_manager.execute_command(container_id, "hostname -I", timeout=30)
        container_ip = ip_result.output.strip().split()[0] if ip_result.output.strip() else "unknown"
        ...
        return jsonify({
            ...
            'access_url': f"http://{container_ip}:3000" if container_ip != "unknown" else "unknown",
            'timestamp': datetime.now().isoformat()
        })
