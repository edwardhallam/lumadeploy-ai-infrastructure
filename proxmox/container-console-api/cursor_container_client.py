...
class CursorContainerClient:
    ...
    def __init__(self, api_base_url: str = None):
        if api_base_url is None:
            import os
            api_base_url = os.getenv("CONTAINER_CONSOLE_API_URL", "http://localhost:5000")
    ...
    def list_containers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if data['success']:
                ...
                print(f"❌ Failed to list containers: {data.get('error', 'Unknown error')}")
                return []
    ...
    def get_container_info(self, container_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.session.get(f"{self.api_base_url}/containers/{container_id}/info")
            response.raise_for_status()
            data = response.json()
            ...
            if data['success']:
                ...
                print(f"❌ Failed to get container info: {data.get('error', 'Unknown error')}")
                return None
    ...
    def test_container_access(self, container_id: int) -> bool:
        ...
        try:
            response = self.session.get(f"{self.api_base_url}/containers/{container_id}/test")
            response.raise_for_status()
            data = response.json()
    ...
    def execute_command(self, container_id: int, command: str, timeout: int = 30) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.api_base_url}/containers/{container_id}/execute",
                json=payload
            )
            response.raise_for_status()
            data = response.json()
    ...
    def deploy_librechat(self, container_id: int) -> bool:
        ...
        try:
            ...
            response = self.session.post(f"{self.api_base_url}/containers/{container_id}/deploy-librechat")
            response.raise_for_status()
            data = response.json()
            
            if data['success']:
                print(f"✅ LibreChat deployment completed!")
                ...
                print(f"\n🎉 Final Status:")
                print(f"   LibreChat Running: {'✅ Yes' if librechat_running else '❌ No'}")
...
def main():
    ...
    if librechat_container:
        ...
        if client.test_container_access(container_id):
            ...
            print(f"1. Deploy LibreChat (automated)")
            print(f"2. Interactive session")
            print(f"3. Test single command")
